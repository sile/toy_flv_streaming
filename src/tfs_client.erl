...
-spec publish(iodata(), inet:port_number(), iodata(), flv:flv()) -> ok | {error, Reason::term()}.
publish(Host, Port, StreamName, {_, FlvEntries}) ->
    case gen_tcp:connect(Host, Port, [binary, {active, false}]) of
        {error, Reason} -> {error, Reason};
        {ok, Socket}    ->
            ok = gen_tcp:send(Socket, ["/publish ", StreamName, " "]),
...
encode_flv_tag(#flv_tag{timestamp = Timestamp, data = Data}) ->
    ...
    [<<Type:8, Timestamp:32, (iolist_size(Bytes)):32>>, Bytes].
